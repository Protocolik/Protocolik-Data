var wikiTextParser = new parseWikiTable("wiki.vg");
...
function getProtocolComments(cb) {
    async.waterfall([
        ...
        transformProtocol.bind(null, function () {
            return {};
        }, function (obj, name, v) {
            obj[name] = v;
            return obj;
...
function extractProtocol(fnPacket, sectionObject, cb) {
    var notStates = R.omit(["content", "Definitions", "Packet format"]);
...
function commentsForPacket(packetText, cb) {
    ...
    var packet = packetText.reduce(function (acc, line) {
    ...
    pdc(packet.before.join("\n"), "mediawiki", "markdown_github", ['-F', __dirname + '/relink.js'], function (err, result) {
        if (err) return cb(err);
        packet.before = result.split("\n");
        pdc(packet.after.join("\n"), "mediawiki", "markdown_github", ['-F', __dirname + '/relink.js'], function (err, result) {
            if (err) return cb(err);
...
// transforms
function transformProtocol(initialState, addToPacket, transformPacket, toOldNames, protocol, cb) {
                ...
                .reduce(function (stateO, direction) {
                    ...
                    stateO[transformedDirection] = Object
                        ...
                        .reduce(function (packetsO, packetName) {
                            ...
                            var transformedPacketName = toOldNames(packetName, transformedState, transformedDirection, transformedPacket ? transformedPacket["id"] : null);
...
function transformPacket(packet, state, direction) {
    ...
    return {
        "id": transformedId,
        "fields": packet["fields"] ? packet["fields"].map(function (field) {
            return transformField(field, state, direction, transformedId);
        }) : null
    };
}
